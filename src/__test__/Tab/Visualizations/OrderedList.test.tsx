/**
 * Module containing tests for the Ordered List visualization
 */
...
const emptyResponseServerUrl = "ws://ordered-list-empty-response.com";
const singleDatasetServerUrl = "ws://ordered-list-single-dataset-response.com";
const multipleDatasetServerUrl =
  "ws://ordered-list-multiple-dataset-response.com";
...
beforeEach(async () => {
  ...
  userEvent.selectOptions(visualizationTypeInput, ["ordered-list"]);
});
...
test("should show 'no data' if no data is received", async () => {
  await setUpMockOrderedList(emptyResponseServerUrl, true);
...
test("should show dataset name and value if data is isMultiple and append is true", async () => {
  ...
  await setUpMockOrderedList(multipleDatasetServerUrl, true, "quantity");
  await goToTab(tabName);
  ...
  const allListItems = screen.getAllByTestId("ordered-list-item");
...
test("should show dataset name and value if data is isMultiple and append is false", async () => {
  ...
  await setUpMockOrderedList(multipleDatasetServerUrl, false, "quantity");
  await goToTab(tabName);
...
test("should display the ordered-list as expected when append data is true", async () => {
  await setUpMockOrderedList(singleDatasetServerUrl, true);
  await goToTab(tabName);
...
test("should display the ordered-list as expected when append data is false", async () => {
  await setUpMockOrderedList(singleDatasetServerUrl, false);
  await goToTab(tabName);
...
test("should display the list item with the set list style type", async () => {
  const newListStyleType = "upper-alpha";
  await setUpMockOrderedList(singleDatasetServerUrl, false);
...
/**
 * Sets up the ordered-list mock visualization
 * @param dataSourceUrl - the mock webscoket URL from which data is to be got
 */
async function setUpMockOrderedList(
