/**
 * Module containing tests for the multiple line Chart visualization
 */
...
beforeEach(async () => {
  ...
  userEvent.selectOptions(visualizationTypeInput, ["multiple-line-chart"]);
});
...
test("should show 'no data' if no data is received", async () => {
  await setUpMockMultiLineChart(emptyResponseServerUrl);
...
test("should show error if data is not isMultiple", async () => {
  await setUpMockMultiLineChart(singleDatasetServerUrl);
  await goToTab(tabName);
  expect(
 ...
 */
async function setUpMockMultiLineChart(dataSourceUrl: string) {
