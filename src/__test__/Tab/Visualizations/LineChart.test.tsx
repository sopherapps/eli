...
const emptyResponseServerUrl = "ws://line-chart-empty-response.com";
const singleDatasetServerUrl = "ws://line-chart-single-dataset-response.com";
const multipleDatasetServerUrl =
  "ws://line-chart-multiple-dataset-response.com";
...
beforeEach(async () => {
  ...
  userEvent.selectOptions(visualizationTypeInput, ["line-chart"]);
});
...
test("should show 'no data' if no data is received", async () => {
  await setUpMockLineChart(emptyResponseServerUrl);
...
test("should show error if data is isMultiple", async () => {
  await setUpMockLineChart(multipleDatasetServerUrl);
  await goToTab(tabName);
  expect(
 ...
 */
async function setUpMockLineChart(dataSourceUrl: string) {
  ...
  // set the color under the line
  const colorUnderCurveTextInput = await screen.findByLabelText(
    /^color of area under line/i
  );
  userEvent.clear(colorUnderCurveTextInput);
  userEvent.type(colorUnderCurveTextInput, "#fffeaf");
}
