...
beforeEach(async () => {
  ...
  userEvent.selectOptions(visualizationTypeSelect, ["text"]);
});
...
test("should ask for font-size, defaulting to 16", async () => {
...
test("should ask for whether to make text bold or not, defaulting to off", async () => {
  const labelPattern = /^bold$/i;
  ...
  const clickedInput = await screen.findByLabelText(labelPattern);

  expect(clickedInput).toBeChecked();
});

test("should ask for whether to make text italic or not, defaulting to off", async () => {
  const labelPattern = /^italic$/i;
  const input = screen.getByLabelText(labelPattern);
  expect(input).toBeInTheDocument();
...
test("should ask for alignment", async () => {
  const labelPattern = /alignment/i;
  const newValues = ["left", "center", "right"];
  const input = screen.getByLabelText(labelPattern);
  expect(input).toBeInTheDocument();
  expect(input).not.toBeRequired();
