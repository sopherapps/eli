/**
 * Module to test the unordered list form
 */
...
test("should ask for style", async () => {
  ...
  expect(styleInput).not.toBeRequired();

  userEvent.selectOptions(styleInput, [newValue]);
  const changedStyleInput = await screen.findByLabelText(labelPattern);

  expect(changedStyleInput).toHaveDisplayValue(newValue);
});
